...
[_QWERTY] = LAYOUT_planck_grid(
    ...
    ITGMENU, KC_LCTL, KC_LALT, KC_LGUI, LOWER,   KC_SPC,  KC_SPC,  RAISE,   KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT
),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case ITGMENU:
      if (record->event.pressed) {
        set_single_persistent_default_layer(_ITG_MENU);
      }
      return false;
      break;
...
bool rgb_matrix_indicators_user(void) {
    if(get_highest_layer(layer_state|default_layer_state) != _ITG_MENU) {
        return false;
    }
    for (uint8_t i = 0; i < RGB_MATRIX_LED_COUNT; i++) {
        // rgb_matrix_set_color(i, RGB_RED);
        uint8_t color_index = itg_layer_colors[i];
        rgb_matrix_set_color(
            i,
            itg_layer_colormap[color_index][0],
            itg_layer_colormap[color_index][1],
            itg_layer_colormap[color_index][2]
        );
    }
    return true;
}
